...
const (
	MAILBOX_TL_TIME = etcd.MAILBOX_TL_TIME / 2
)
...
func (p *Player) Init() {
	p.Actor.Init()
	p.RegisterTimer((MAILBOX_TL_TIME/2)*time.Second, p.UpdateLease)
	p.RegisterTimer(5*time.Second, p.SaveDB)
...
// 加载玩家结束
func (p *Player) Load_Player_DB_Finish(ctx context.Context, data orm.PlayerData) {
	p.isInGame = true
	p.PlayerData = data
	//加载到地图
	p.LoginFinish()
